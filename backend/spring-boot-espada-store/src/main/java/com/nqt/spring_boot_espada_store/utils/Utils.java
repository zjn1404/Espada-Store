...
public class Utils {
    ...
    public VerifyCode generateVerifyCode(User user) {
        Date expiryDate = new Date(Instant.now()
                .plus(VERIFY_CODE_VALID_DURATION, ChronoUnit.SECONDS)
                .toEpochMilli());
    ...
    public void sendNewProductNotification(User user, Product product)
            throws MessagingException, UnsupportedEncodingException {
        ...
        mailContent += "<p>Happy " + dayOfWeek + "! " + product.getName()
                + " is here at last, and we wanted to make sure you'd be the first to know.</p>";
        ...
        mailContent += "<p>You can order the product <a href=\"" + CLIENT_HOME_PAGE + "/product/" + product.getId()
    ...
    private MimeMessage configMessage(User user, String sender, String subject, String mailContent)
            throws MessagingException, UnsupportedEncodingException {
